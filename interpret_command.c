/*
This file contains the code to interpret the command, i.e. the line entered. 
It parses the line entered into different commands on the basis of semicolons. 
Then if it contains a pipe, it is executed by a pipe_handler. 
Otherwise break into different arguments based on spaces. In case, the command entered is exit,
echo or pwd, the command is executed without calling any external function.
For other built-ins, like cd, ls and pinfo, external functions are used. 
It also executes external commands using exec in both foreground and background manner. 
It also handles redirection using a redirect_handler function
